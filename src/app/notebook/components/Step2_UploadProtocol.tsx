...
export function Step2_UploadProtocol({ protocolTitle, onTranscribed, onBack, addToast }: Step2Props) {
  ...
  const processFile = useCallback(
    async (file: File) => {
      ...
      // Document types — extract text via API
      const isImage = ["image/jpeg", "image/png", "image/webp"].includes(processedFile.type);
      const isPdf = processedFile.type === "application/pdf";
      const isDocx =
        processedFile.type === "application/vnd.openxmlformats-officedocument.wordprocessingml.document" ||
        processedFile.name.endsWith(".docx");
      ...
      throw new Error("Unsupported file type: " + processedFile.type);
    },
    []
  );
    ...
    async (files: File[]) => {
      ...
      try {
        ...
        if (results.length > 0) {
          ...
          addToast("Files processed successfully!", "success");
        }
      } catch (err) {
  ...
  const handlePasteSubmit = () => {
    ...
    addToast("Text submitted!", "success");
  };
        ...
        try {
          ...
          if (checkData.status === "submitted" && checkData.filesData) {
            ...
            for (const fileInfo of checkData.filesData) {
              ...
              if (ocrRes.ok && ocrData.markdown) {
                results.push(ocrData.markdown);
              }
            }
            if (results.length > 0) {
  ...
  // If we have transcribed text, show it for review
  if (transcribedText) {
    return (
      <div className="mx-auto max-w-2xl space-y-6">
        <div className="text-center space-y-2">
          ...
          >
            Review <span style={{ color: "#00ac73" }}>Transcription</span>
          </h2>
          <p className="text-sm" style={{ color: "#6b7280" }}>
            Review and edit the extracted text before proceeding.
          </p>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <Button
            type="button"
            onClick={() => onTranscribed(transcribedText)}
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      <div className="text-center space-y-2">
        <h2
          ...
          style={{ fontFamily: "Pulp, sans-serif" }}
        >
          Upload <span style={{ color: "#00ac73" }}>{protocolTitle}</span>
        </h2>
        <p className="text-sm" style={{ color: "#6b7280" }}>
          Upload your protocol content using any of these methods.
        </p>
      </div>
      ...
      {tab === "upload" && (
        <FileDropZone
          ...
          label="Drop protocol files here"
          sublabel="PDF, JPEG, PNG, HEIC, DOCX (max 20 MB)"
        />
      )}
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border p-1" style={{ background: "#fff", borderColor: "#e5e7eb" }}>
            <textarea
              ...
              placeholder="Paste your protocol text here, or type it manually..."
          ...
          <Button
            ...
            style={{
              background: pasteText.trim() ? "#00ac73" : "#d1d5db",
              color: "#fff",
            }}
          >
            Submit Text
          ...
          {qrDataUrl ? (
            <>
              ...
              <p className="text-sm text-center" style={{ color: "#6b7280" }}>
                Scan this QR code with your phone to take photos of your protocol.
                <br />
                <span className="font-medium" style={{ color: "#00ac73" }}>
                  Waiting for photos...
                </span>
              </p>
              {loading && (
                <div className="flex items-center gap-2">
                  <div className="h-4 w-4 animate-spin rounded-full border-2" style={{ borderColor: "#d1fae5", borderTopColor: "#00ac73" }} />
                  <span className="text-sm" style={{ color: "#00ac73" }}>Processing received photos...</span>
                </div>
              )}
            </>
          ) : (
            <>
              ...
              <p className="text-sm text-center" style={{ color: "#6b7280" }}>
                Generate a QR code to scan with your phone and take photos of your protocol.
              </p>
              <Button
                type="button"
